...
Game.prototype.init = function (state)
{
  ...
  function level_finished(e, state) {
    game.post(function () {
      if (state == 'DIED') {
	game.init(2);
      } else {
  ...
  case 0:
    ...
    break;
  case 1:
    this.health_node = this.addElement(new Rectangle(10, 10, 100, 20));
    this.updateHealth(0);
    this.left_node = this.addElement(new Rectangle(this.frame.width-120, 10, 100, 20));
    this.updateLeft(0);
  ...
  case 2:
    this.scene = new Title(this);
    this.scene.init("<b>You Died!</b><p>Press Enter to restart.");
    this.scene.changed.subscribe(title_changed);
    break;
  case 3:
    this.scene = new Title(this);
...
Game.prototype.updateLeft = function (d)
{
  // [GAME SPECIFIC CODE]
  this.left_node.innerHTML = ('Left: '+d);
};

Game.prototype.updateHealth = function (h)
{
  // [GAME SPECIFIC CODE]
  this.health_node.innerHTML = ('Health: '+h);
};
