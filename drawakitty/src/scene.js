...
{
  Scene.call(this, game);
  this.buf = createCanvas(game.screen.width,
			  game.screen.height);
  this.ctx = getEdgeyContext(this.buf)
}
...
Level.prototype.init = function ()
{
  ...
  this.ctx.fillRect(0, 0, this.buf.width, this.buf.height);
  this.x0 = 0;
  this.y0 = 0;
  this.mouse = false;
};
...
Level.prototype.update = function ()
{
  ...
  for (var i = 0; i < this.colliders.length; i++) {
    ...
    if (obj1.alive && obj1.scene === this && obj1.bounds !== null && this.mouse) {
      if (obj1.bounds.contains(this.x0, this.y0) ||
	  obj1.bounds.move(obj1.vx, 0).contains(this.x0, this.y0)) {
	this.changed.signal('LOST', 0);
      }
    }
  }

  if (this.start && rnd(10) == 0) {
    var vx = rnd(20)-10;
    var s = rnd(20)+10;
    if (vx != 0) {
      var bounds = new Rectangle((0 < vx)? 0 : this.buf.width-s, rnd(this.buf.height-s), s, s);
      this.addObject(new Actor(bounds, vx));
    }
  }
...
Level.prototype.mousedown = function (x, y, button)
{
  // [OVERRIDE]
  this.start = true;
  this.x0 = x;
...
Level.prototype.mouseup = function (x, y, button)
{
  // [OVERRIDE]
  this.mouse = false;
};
