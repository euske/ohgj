...
Game.prototype.init = function (state, jpy, usd, usd_rate)
{
  // [OVERRIDE]
  // [GAME SPECIFIC CODE]
  ...
  function level_changed(e, arg, jpy, usd, usd_rate) {
    switch (arg) {
    case 'LOST': game.post(function () { game.init(3, jpy, usd, usd_rate); }); break;
    }
  }
  switch (state) {
  case 0:
    this.scene = new Title(this);
    this.scene.init('<b>Exchangeruga</b><br>1HGJ, Jul 12th, euske<p><b>How to Play:</b><br>Collect 1 USD bill or 100 JPY bill.<br>Try not to lose both currencies.<br>The exchange rate keeps changing.<p><b>Press Enter to start.</b>');
  ...
  case 3:
    this.scene = new Title(this);
    var gain_jpy = jpy+usd*usd_rate-1000;
    var gain_usd = usd+jpy/usd_rate-10;
    this.scene.init('<b>You Lost!</b><p><b>JPY: 1000 -> '+jpy+' (100JPY='+(Math.floor(10000/usd_rate)/100)+'USD)<p>USD: 10 -> '+(Math.floor(usd*100)/100)+' (1USD='+usd_rate+'JPY)<p>Gain: '+Math.floor(gain_jpy)+'JPY / '+(Math.floor(gain_usd*100)/100)+'USD</b><p>Press Enter to restart.');
    this.scene.changed.subscribe(title_changed);
