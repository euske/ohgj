...
Game.prototype.init = function (state, arg)
{
  // [OVERRIDE]
  // [GAME SPECIFIC CODE]
  ...
  function level_finished(e, arg) {
    game.post(function () { game.init(2, arg); });
  }
  switch (state) {
  case 0:
    this.scene = new Title(this);
    this.scene.init('<span class=red>赤</span> versus <span class=blue>青</span><p>Made with JSCS<p>Press Enter to start.');
  ...
  case 1:
    ...
    this.audios.start.currentTime = 0;
  ...
  case 2:
    this.scene = new Title(this);
    if (arg == 'RED') {
      color = '<b><span class=red>赤</span> Won!</b>';
    } else if (arg == 'BLUE') {
      color = '<b><span class=blue>青</span> Won!</b>';
    } else {
      color = '<b>DRAW!</b> (is that even possible?)';
    }
    this.scene.init(color+'<p>Press Enter to restart.');
    this.scene.changed.subscribe(title_changed);
    this.audios.start.currentTime = 0;
    this.audios.start.play();
    this.music = null;
